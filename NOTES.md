...
## Two options for dealing with current race conditions between generateAnswers (which sets state) and getClues which relies on the answers generated by generateAnswers

**One**: 
- refactor generateAnswers so it returns the generated answers instead of setting state
- it is actually populateClues that is called by generateAnswers that calls the state setters
- this value can then be used by getClues locally

**Two**: 
- add a useEffect to our useClueFetch hook. Instead of returning getClues, we return isFetching and setIsFetching. In our click handler we call setIsFetching(true). This triggers the useEffect in our hook which calls getClues.
- we would also have to pass in clueList to the clues state in the hook
- in our useEffect, we get the clues, the the local clues state and set isFetching to false.
- a useEffect in SolveGrid.tsx the triggers when clues updates and sets the clueList to clues.

- I think option one makes more sense
